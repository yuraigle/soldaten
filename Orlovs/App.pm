...
P::hook {
  ...
  if ($prompt_uc =~ m/\d+H\s+\d+M/sxm) {
    ...
    if ( $prompt_uc =~ m/Лч:(\d+)/sxm ) { $LC = $1 } else { $LC = 9 }
    if ( $prompt_uc =~ m/ОЗ:(\d+)/sxm ) { $OZ = $1 } else { $OZ = 0 }
    if ( $prompt_uc =~ m/Пн:(\d+)/sxm ) { $PN = $1 } else { $PN = 0 }
    if ( $prompt_uc =~ m/Бм:(\d+)/sxm ) { $MO = $1 } else { $MO = 0 }
    if ( $prompt_uc =~ m/Ог:(\d+)/sxm ) { $OG = $1 } else { $OG = 0 }
  }
